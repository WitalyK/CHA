...
Fname1x:=Fname1x+'Журнал регистрации передач от '+Fname2x;
...
ix:=2;
      ...
      Excel.Cells[ix, 1].Value:='Время выхода в эфир';
      Excel.Cells[ix, 2].Value:='Название передачи';
      Excel.Cells[ix, 3].Value:='Возр. огр';
      Excel.Cells[ix, 5].Value:='Хронометраж рекламы';
      Excel.Cells[ix, 6].Value:='Хронометраж передач';
      Excel.Cells[ix, 7].Value:='Автор/ведущий передачи';
      Excel.Cells[ix, 8].Value:='Тема передачи';
      Excel.Cells[ix, 9].Value:='Участники передачи';
      ix:=ix+1;
      tx:=0;
      cx:=0;
      PrRek:=false;
      ...
      Excel.Cells[ix, 2].Value:='Ожидание времени '+copy (name1x,11,7);
      ...
      begin
      Excel.Cells[ix, 2].Value:='Ожиданиеи времени '+copy (name1x,11,7);
      Excel.Cells[ix+1, 1].Value:=copy (name1x,11,7);
...
//      Excel.Cells[ix, 6].Value:='Аппаратнпя';
      Excel.Cells[ix, 2].Value:='Прямой эфир';
//      Excel.Cells[ix, 4].Value:='';
      ...
      if pos ('Рекламный блок',name1x)>0 then
      begin
        if pos ('начало',name1x)>0 then
        begin
          if not PrRek then
          begin
            cx:=28;
          ...
          begin
            ...
            try
              Application.Title := 'Оп-ля, ошибочка тай!';
              ShowMessage('ВНИМАНИЕ!!! Нет конца рекламного блока '+NumbeRek+#13#10+'Исправляйте плейлист!');
            finally
...
//      Excel.Cells[ix, 4].Value:='';
        ...
        begin
          ...
          begin
            savedTitle := Application.Title;
            try
              Application.Title := 'Ой, ошибочка вышла!';
              ShowMessage('ВНИМАНИЕ!!! Нет начала рекламного блока '+copy(name1x,26,3)+#13#10+'Исправляйте плейлист!');
            finally
              Application.Title := savedTitle;
            end;
            Excel.DisplayAlerts := False;
...
Excel.Selection.Borders.LineStyle := 1; // стиль линии сплошная
Excel.Selection.Borders.Weight := 1;// толщина линии
Cell1:=Excel.Cells[ix+4, 1];
Cell2:=Excel.Cells[ix+4, 9];
Excel.Range[Cell1, Cell2].Merge;
Excel.Cells[ix+4, 1].Font.size := 14;
Excel.Cells[ix+4, 1].Value:='Выпускающий режиссер ________________________________/_______________________________/';
Excel.Range['A1:E1000'].Select;
Excel.Selection.HorizontalAlignment:=3;
Excel.Selection.NumberFormat := 'чч:мм:сс,00';
