...
def generate_description_from_cdp(show_cdp_neighbors_command_file):
    with open(show_cdp_neighbors_command_file) as command:
        command_str = command.read()
    return {intf: 'description Connected to {} port {}'.format(device, port) for device, intf, port in
            findall(r'(\w+) +(\w+ \d/\d+).+ (\w+ \d/\d+)', command_str)}
